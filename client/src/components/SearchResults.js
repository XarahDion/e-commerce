...
const SearchResults = ({
    ...
    const { ids, allProducts, setSearchProducts, setSearch, seeAll } =
    ...
    if (items && active) {
        results = itemNames.filter((itemName) =>
                ...
                <ResultsList>
                    {
                        ...
                        firstResults.map((result, i) => {
                            ...
                            {
                                let currentName = itemNames.indexOf(
                                    combinedResults[selectedResultsIndex]
                                ); // using current index of keyboard, get name of product
                            ...
                            return (
                                <>
                                    ...
                                    >
                                        <Before>
                                            {before(result, searchInputs[0])}
                                        </Before>
                                        <Match>
                                            {match(result, searchInputs[0])}
                                        </Match>
                                        <After>
                                            {after(result, searchInputs[0])}
                                        </After>
                                    </Result>
                                </>
                            );
